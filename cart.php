      ...
      <?php else: ?>
        <table class="table table-bordered table-condensed table-striped">
          ...
          <tbody>
            <?php
            if(!empty($items)){
              foreach($items as $item){ // for each item in item array i will loop
                if($item != null){
                  $product_id = $item['id'];
        ...
        <!-- Total items sum cost of items  -->
        <table class="table table-bordered table-condensed text-right">
          <leyend><h3>Totals</h3></leyend><hr>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <form action="thankyou.php" method="post" id="payment-form">
          ...
          <input type="hidden" name="description" value="<?=$item_count.' item'.(($item_count>1)?'s':'').' de Ingenieriajrm.';?>">
...
<script>
  function back_address(){
      ...
      jQuery("#back_button").css('display','noe');
  ...
  function check_address(){
    ...
    jQuery.ajax({
      url : '/ecommerce2/admin/parsers/check_address.php',
      method : 'POST',
      data : data,
